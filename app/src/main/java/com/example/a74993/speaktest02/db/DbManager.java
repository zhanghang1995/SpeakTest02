...
public class DbManager {
    ...
    public void addUser(String deviceId,String username,String getuptime,String breaktime,String lunchtime,String dinnertime)
    {
        ...
        try {
            db.execSQL("INSERT INTO " + Constant.USERTABLE+"(username,deviceId,getuptime,breaktime,lunchtime,dinnertime)"
    ...
    public void updateTime(String type,String value,String device_id,Context context)
    {
        ...
        db.beginTransaction(); // 开始事务
        try {
            db.execSQL("UPDATE " + Constant.USERTABLE+" SET"+"'"+type+"'"+"="+"'"+value+"'"+" WHERE "+"'"+device_id+"'"+" = "+"'"+ ToolsUtils.getUserDeviceID(context)+"'");
            // 带两个参数的execSQL()方法，采用占位符参数？，把参数值放在后面，顺序对应
            // 一个参数的execSQL()方法中，用户输入特殊字符时需要转义
            // 使用占位符有效区分了这种情况
     ...
     * @return Cursor
     */
    public void queryTheType(Context context,int type)
    {
        LogInfo.e("SQLITE","DBManager --> queryTheCursor");
        Cursor c = db.query( DbHelper.TABLE_NAME,null,
                "deviceId ="+"'"+ ToolsUtils.getUserDeviceID(context)+"'",null,null, null, null);
        ...
        {
            if(type==1){
            int breaktype = c.getColumnIndex("breaktype");
            String ctype = c.getString(breaktype);
